...
_mounts.test = (app, name, middleware, controllers) => {
	setupPageRoute(app, `/${name}`, [], controllers.test.home);
};
...
module.exports = async function (app, middleware) {
	...
	const remountable = ['admin', 'category', 'topic', 'post', 'users', 'user', 'groups', 'tags', 'test'];
